...
// TimeUnderMutex is a time.Time protected by a sync.RWMutex.
type TimeUnderMutex struct {
	sync.RWMutex
	t time.Time
}

// Get returns the stored time.Time value.
func (s *TimeUnderMutex) Get() time.Time {
	s.RLock()
	defer s.RUnlock()
	return s.t
}
...
func (s *TimeUnderMutex) SetIfZero(t time.Time) {
	s.Lock()
	defer s.Unlock()
	if s.t.IsZero() {
		s.t = t
