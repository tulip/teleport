...
func (s *Server) IsLockedOut(id srv.IdentityContext) error {
	lock, err := s.lockWatcher.GetLockInForce(srv.ComputeLockTargets(s, id)...)
	// TODO(andrej): Handle the special case of ConnectionProblem (stale lock views).
	if err != nil {
...
func (s *Server) handleSessionRequests(ctx context.Context, ccx *sshutils.ConnectionContext, identityContext srv.IdentityContext, ch ssh.Channel, in <-chan *ssh.Request) {
	...
	ctx, scx, err := srv.NewServerContext(ctx, ccx, s, identityContext) //s.proxyMode)
	if err != nil {
