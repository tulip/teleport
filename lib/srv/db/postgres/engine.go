...
func (e *Engine) connect(ctx context.Context, sessionCtx *common.Session) (*pgproto3.Frontend, *pgconn.HijackedConn, error) {
	...
	if err != nil {
		if trace.IsAccessDenied(common.ConvertError(err)) && sessionCtx.Server.IsRDS() {
			return nil, nil, trace.AccessDenied(`Error connecting to database:

  %v

Make sure Teleport has IAM policy attached that allows it to connect to your RDS instance:
