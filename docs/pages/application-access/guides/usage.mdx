...
Join token is required to authorize a Teleport Application Proxy service to
...
a role that allows it to see it.
...
endpoint because application service always connects back to the cluster over
a reverse tunnel.
...
teleport:
  ...
  # because app service always connects to the cluster over reverse
  # tunnel.
  auth_servers:
...
For testing and debugging purposes we provide a builtin debug app. It can
be turned on use `debug_app: true`.
...
address. To override the public address, specify `public_addr` field:
...
Some applications are available on a subdirectory. Wxamples include the
...
They can be viewed by navigating to a cluster and selecting Applications tab.
...
force a log out by hitting the `/teleport-logout` endpoint:
