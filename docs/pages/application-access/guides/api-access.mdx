...
And connect it to your Teleport cluster by adding the following section in
...
will use client certificate to authenticate with Teleport.

As Grafana API requires authentication, let's update the `curl` command to
...
The app's X.509 certificate will expire on its own after some TTL governed by
your user's role. You can also remove it explicly:
...
Teleport provides a way to see the information about the application you're
